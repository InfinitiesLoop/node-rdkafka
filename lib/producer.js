...
Producer.prototype.produceMany = function(msg, cb) {
  if (!this._isConnected) {
    throw new Error('Producer not connected');
  }
  ...
  this.outstandingMessages += msg.messages.length;
  this.sentMessages += msg.messages.length;
  ...
  var produceOpts = {
    partition: partition,
    messages: msg.messages, //!Buffer.isBuffer(msg.message) ? new Buffer(msg.message) : msg.message,
    key: msg.key || null,
    opaque: msg.opaque || null
  };

  this._client.produceMany(produceOpts, topic, function clientProduce(err) {
    if (err) {
      self.emit('error', LibrdKafkaError.create(err));
    }
    if (cb) {
